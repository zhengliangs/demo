...
public class LazyMessage {
    ...
    public void convertAndSend(){
        MessageBody messageBody = new MessageBody("order.convertAndSend", 0);
        messageBody.setTime(System.currentTimeMillis());
        // 将消息发送到业务队列，但是没有消费者监听，消息在超时后，会被路由到业务队列绑定的延迟队列中，然后被延迟队列的消费者监听到
        rabbitTemplate.convertAndSend(LazyConfig.ORDER_EXCHANGE, LazyConfig.ORDER_ROUTING_KEY, messageBody, message -> {
            // 设置超时时间 2000ms
