...
public class RoutingConfig {
    ...
    public Binding routingBinding1(){
        return BindingBuilder.bind(routingQueue1()).to(directExchange()).with(ROUTING_KEY_1);
    }

    // 将队列routingQueue2绑定到交换器上，并指定binding key为routing2
    @Bean
    public Binding routingBinding2(){
        return BindingBuilder.bind(routingQueue2()).to(directExchange()).with(ROUTING_KEY_2);
    }

    // 将队列routingQueue3绑定到交换器上，并指定binding key为routing1
    @Bean
    public Binding routingBinding3(){
        return BindingBuilder.bind(routingQueue3()).to(directExchange()).with(ROUTING_KEY_1);
    }
